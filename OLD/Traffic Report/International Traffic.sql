...
-- create table CountryCode ( CountryCode int, CountryName String) engine = MergeTree() order by CountryCode;
...
group by Date,callReference,CallingNumber,CalledNumber,RoamingNumber,CountryCode,Route--,calledPartyNumber--callDuration,,callingPartyNumber
order by Date,CallingNumber,CalledNumber
        )any left join
(
    select CountryName, toString(CountryCode) CountryCode from CountryCode
    )using CountryCode;
