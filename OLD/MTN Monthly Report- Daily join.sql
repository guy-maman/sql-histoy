...
select toDate(EventDate) MTN
        ,round(sum(toUnixTimestamp(chargeableDuration))/60) On_Net
from mediation.ericsson
where toYear(EventDate) = (:year)
        and toMonth(EventDate) = (:month)
--         EventDate >= toDateTime(:from_v)
    ...
    from (
            ...
            from (
                  select EventDate,
                         ...
                         callingPartyNumber,
                         calledPartyNumber,
                         max(toUnixTimestamp(chargeableDuration)) chargeableDuration,
                         max(incomingRoute)
                  from mediation.ericsson
                  where toYear(EventDate) = (:year)
                    and toMonth(EventDate) = (:month)
...
--                     and EventDate < toDateTime(:to_v)
                  ...
                  group by EventDate, eosInfo, type, callingPartyNumber, calledPartyNumber
                     )GROUP BY MTN
                      )GROUP BY MTN
             ) any
             left join
         (
            select toDate(EventDate)                                MTN
                              , round(sum(chargeableDuration) / 60) International_Incoming
            from (
                  select EventDate,
                         eosInfo,
                         type,
...
from (
        select toDate(EventDate)                                MTN
                          , round(sum(toUnixTimestamp(chargeableDuration)) / 60) Orange_To_MTN
        from (
              select EventDate,
                     eosInfo,
                     type,
...
--                 and EventDate < toDateTime(:to_v)
         ...
         (
            select toDate(EventDate)                                MTN
                              , round(sum(toUnixTimestamp(chargeableDuration)) / 60) MTN_To_Orange
            from (
                  select EventDate,
                         eosInfo,
                         type,
...
--         recordOpeningTime >= toDateTime(:from_v)
