select  Top 10 CountryName,round(sum(duration)/60,2) INTL_Incoming
from (
         ...
         select case
                    when substring(callingPartyNumber, 1, 2) = '14'
                        then '231'
                              ...
                              when substring(callingPartyNumber, 3) like ('1%') then substring(callingPartyNumber, 3, 4)
...
order by INTL_Incoming desc

-------------------------------------------------------------------------------------------------------------

select  Top 10 CountryName,round(sum(duration)/60,2) INTL_Outgoing
from (
