...
(
    ...
    from (
             ...
             from (
                   ...
                   GROUP BY MTN
                   union all
                   select MTN, round(sum(International_Outgoing) / 60) International_Outgoing
                   from (
                         select toDate(EventDate)                         MTN,
                                ...
                                callingPartyNumber,
                                translatedNumber,
                                toUnixTimestamp(chargeableDuration) / count() International_Outgoing
                         from ericsson
                         where EventDate >= toDateTime(:from_v)
                           and EventDate < toDateTime(:to_v)
                         ...
                         group by MTN, dateForStartOfCharge, timeForStartOfCharge, timeForStopOfCharge,
                                  callingPartyNumber, translatedNumber, chargeableDuration
                            )GROUP BY MTN
                      )GROUP BY MTN
             ) any
             left join
