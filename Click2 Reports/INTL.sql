...
/*
create table default.INTL (Operator Nullable(String),Direction Nullable(String),callReference Nullable(String),Date DateTime
...
------------------------------------------------------  MTN  -------------------------------------------------
-- truncate table default.INTL
-- truncate table default.CountryCodes
...
left any join
(
    select CountryName, toString(CountryCode) CountryCode from default.CountryCodes
...
select  Operator, Direction,round(sum(callDuration)/60,2)
from    default.INTL
...
from (
      ...
      order by callDuration desc
      union all
      select Top 14 3 ind, 'Merged' Operator, Direction, CountryName, round(sum(callDuration)/60,2) callDuration
      from default.INTL
      where toYear(Date) = (:year)
        and toMonth(Date) = (:month)
        and CountryName not in ('Liberia')
--     and Operator = 'MTN'
      ...
      order by callDuration desc
      union all
      select Top 14 6 ind, 'Merged' Operator, Direction, CountryName, round(sum(callDuration)/60,2) callDuration
      from default.INTL
      where toYear(Date) = (:year)
        and toMonth(Date) = (:month)
        and CountryName not in ('Liberia')
