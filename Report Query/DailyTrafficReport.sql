...
insert into Daily_Traffic_Liberia
--ORANGE
select  Operator,Date,sum(On_Net)*1.01 On_Net,sum(International_Incoming)*1.01 International_Incoming
        ,sum(International_Outgoing)*1.01 International_Outgoing,sum(Orange_To_MTN) Orange_To_MTN
        ,sum(MTN_To_Orange) MTN_To_Orange,sum(DATA)*1.01 DATA
from    mediation.Daily_Traffic_d
where   toYear(Date) = (:year)
    and toMonth(Date) = (:month)
        and Operator = 'ORANGE'
...
--MTN
select  Operator,Date,sum(On_Net) On_Net,sum(International_Incoming) International_Incoming
        ,sum(International_Outgoing)*1.07 International_Outgoing,sum(Orange_To_MTN) Orange_To_MTN
        ,sum(MTN_To_Orange) MTN_To_Orange,sum(DATA) DATA
from    mediation.Daily_Traffic_d
where   toYear(Date) = (:year)
    and toMonth(Date) = (:month)
